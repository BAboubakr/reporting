...
const ALIASES={
  'AFRY':['afry'],Artelia:['artelia'],Tractebel:['tractebel','engie'],
  'Mott MacDonald':['mott macdonald','mott macdonald group'],'WSP':['wsp'],Worley:['worley'],
  Egis:['egis'],'ILF Consulting Engineers':['ilf consulting engineers','ilf'],DNV:['dnv'],
  NOVEC:['novec'],'INGEMA':['ingema'],'JESA':['jesa','jesa s.a','jesa sa']
};
...
const moroccoRelevant=s=>s.morocco===true||/morocco|maroc|masen|onee|anre|ocp|rabat|casablanca|laayoune|dakhla|tanger|fes|fez|oujda|kenitra|chefchaouen|taza|guercif|ouarzazate|ifrane|benguerir|phosboucraa/.test(textOf(s));
...
async function renderCompetitors(filter='all'){
 ...
 const live=signals.map(s=>{const competitor=detectCompetitor(s);if(!competitor||daysOld(s.published||s.detected)>30)return null;return {competitor,type:s.signalType||'Market movement',theme:(s.categories||['Energy transition'])[0],signal:s.title||s.headline||s.summary,priority:String(s.fichtnerRelevance||'').toUpperCase()||(s.relevanceScore>=80?'HIGH':s.relevanceScore>=60?'MEDIUM':'WATCH'),source:s.source||'Atlas collector',url:s.url,date:s.published||s.detected,morocco:moroccoRelevant(s),relationship:'Competitor'};}).filter(Boolean);
 const curated=seed.map(s=>{const competitor=detectCompetitor(s)||s.competitor;if(!competitor||daysOld(s.date)>365)return null;return {...s,competitor,morocco:moroccoRelevant(s)};}).filter(Boolean);
 ...
 if(!items.length){const selected=filter==='all'?'the monitored competitors':filter;list.innerHTML=`<div class="empty-state"><strong>No recent signal for ${selected}.</strong><p>Atlas is monitoring this competitor continuously. Global activity is kept separate from Morocco-specific intelligence.</p></div>`;return;}
