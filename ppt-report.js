...
function addSignalCard(slide, s, x, y, w, h) {
  ...
  slide.addText(s.headline||s.title||'Untitled signal',{x:x+.2,y:y+.5,w:w-.4,h:.48,fontSize:13,bold:true,color:'173B37',margin:0,fit:'shrink'});
  ...
  slide.addText(why,{x:x+.2,y:y+1.4,w:w-.4,h:h-1.62,fontSize:9.5,color:'35464A',margin:0,fit:'shrink'});
}
async function generateStructuredPpt() {
  const status=document.getElementById('reportStatus');
  if(status) status.textContent='Generating structured Atlas intelligence report…';
  try {
    const {signals=[]}=await import('./data/signals.js?report=20260825-3');
    ...
    const pptx=new Pptx();
    pptx.layout='LAYOUT_WIDE'; pptx.author='Atlas'; pptx.company='Fichtner';
    ...
    pptx.lang='en-US';
    let s=pptx.addSlide(); s.background={color:'173B37'};
    ...
    s=pptx.addSlide(); addHeader(s,'01 · Executive signal','What requires attention now',`${high} high-priority signals · ${primary} primary/official evidence records · ${usable.length} total structured signals`);
    ranked.slice(0,4).forEach((item,i)=>addSignalCard(s,item,.55+(i%2)*6.05,1.9+Math.floor(i/2)*2.45,5.7,2.15));
    s=pptx.addSlide(); addHeader(s,'02 · Market pulse','Activity by theme','Signal volume from the same structured dataset used by Atlas monitoring.');
    const max=Math.max(1,...themes.map(t=>t[1]));
    themes.forEach(([theme,count],i)=>{const y=1.95+i*.65;s.addText(theme,{x:.7,y,w:3.2,h:.22,fontSize:10,bold:true,color:'294B47',margin:0,fit:'shrink'});s.addShape('roundRect',{x:3.9,y:y+.01,w:6.2,h:.2,rectRadius:.04,line:{color:'DCE4E2',transparency:100},fill:{color:'E8EFED'}});s.addShape('roundRect',{x:3.9,y:y+.01,w:6.2*(count/max),h:.2,rectRadius:.04,line:{color:'2C6E68',transparency:100},fill:{color:'2C6E68'}});s.addText(String(count),{x:10.35,y:y-.02,w:.55,h:.25,fontSize:10,bold:true,color:'173B37',align:'right',margin:0});});
    ...
    s=pptx.addSlide(); addHeader(s,'03 · Signal register','Priority intelligence','The strongest signals with evidence traceability and Fichtner relevance.');
    ranked.slice(0,5).forEach((item,i)=>addSignalCard(s,item,.55,1.8+i*.95,12.0,.78));
    ranked.slice(0,5).forEach((item,i)=>s.addText(`${item.source||'Source'} · ${item.url}`,{x:6.8,y:1.8+i*.95+.5,w:5.2,h:.16,fontSize:7,color:'647477',margin:0,fit:'shrink'}));
    s=pptx.addSlide(); addHeader(s,'04 · Strategic implications','What Atlas suggests','The report separates evidence from interpretation so the next action is visible.');
    ranked.slice(0,4).forEach((x,i)=>{const y=1.9+i*1.15;s.addText(String(i+1).padStart(2,'0'),{x:.65,y,w:.45,h:.35,fontSize:16,bold:true,color:'2C6E68',margin:0});s.addText(x.headline||x.title,{x:1.3,y,w:4.5,h:.3,fontSize:11,bold:true,color:'173B37',margin:0,fit:'shrink'});s.addText(x.whyItMatters||x.summary||'Review the source and assess potential commercial relevance.',{x:5.9,y:y-.02,w:6.0,h:.55,fontSize:9.5,color:'4C5B5F',margin:0,fit:'shrink'});});
    s=pptx.addSlide(); addHeader(s,'05 · Evidence','Source traceability','Every highlighted signal links back to its original source.');
    ranked.slice(0,8).forEach((item,i)=>{const y=1.85+i*.55;s.addText(dateLabel(item.published),{x:.65,y,w:1.05,h:.18,fontSize:7.5,color:'657477',margin:0});s.addText(item.source||'Unknown source',{x:1.85,y,w:2.0,h:.18,fontSize:8.5,bold:true,color:'294B47',margin:0,fit:'shrink'});s.addText(item.headline||item.title,{x:3.95,y,w:5.7,h:.18,fontSize:8.5,color:'35464A',margin:0,fit:'shrink'});s.addText(item.url,{x:9.7,y,w:2.75,h:.18,fontSize:7,color:'2C6E68',margin:0,fit:'shrink'});});
    await pptx.writeFile({fileName:'Atlas_Morocco_Renewable_Intelligence_Brief.pptx'});
    if(status) status.textContent='New structured PowerPoint generated from Atlas signals.';
  } catch(err){console.error(err);if(status) status.textContent=`PowerPoint generation failed: ${err.message}`;}
}
function install(){
  ...
  // Capture phase is intentional: it prevents the legacy app.js click handler from generating the old report.
  replacement.addEventListener('click',e=>{e.preventDefault();e.stopImmediatePropagation();generateStructuredPpt();},true);
}
