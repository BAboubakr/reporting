...
ENRICHMENT_ENGINE_VERSION='5.1-entity-aware-adaptive-market-entry'
...
KNOWN_ENTITIES={
    ...
    'DNV':'DNV', 'INGEMA':'INGEMA', 'RINA':'RINA', 'ONEE':'ONEE', 'MASEN':'MASEN', 'ANRE':'ANRE',
...
def infer_project(signal):
    ...
    if 'offshore' in text and 'wind' in text and any(x in text for x in ('feasibility','study','first offshore')):return 'Morocco Offshore Wind Feasibility Study'
...
def make_queries(signal,level):
    title=signal.get('title',''); entities=signal.get('entities') or []; cats=signal.get('categories') or []; project=infer_project(signal) or ''; refs=signal.get('contractReferences') or []; queries=[]
    ...
    if any(x in t for x in ['office','subsidiary','market entry','local entity','sarl','expands presence']):
        queries += ['Morocco '+title+' official','Morocco '+title+' subsidiary','Morocco '+title+' Casablanca office','Morocco engineering consulting market entry '+ ' '.join(entities[:2])]
...
def enrichment_is_weak(signal,level_num):
    e=signal.get('enrichment') or {}; source_count=int(e.get('sourceCount') or len(e.get('sources') or [])); facts=e.get('facts') or []; entities=signal.get('entities') or e.get('entities') or []; min_sources={1:2,2:4,3:6,4:8}.get(level_num,0)
...
def fallback_enrich(signal,evidence):
    text=' '.join([signal.get('title',''),signal.get('summary','')]+[f"{x.get('title','')} {x.get('snippet','')} {x.get('source','')}" for x in evidence]);n=norm(text);facts=[];project=infer_project(signal);refs=signal.get('contractReferences') or infer_contract_refs(signal,evidence)
    if 'rina' in n and any(x in n for x in ('morocco','maroc','casablanca')):
        facts.append({'claim':'Public sources identify RINA as establishing a Moroccan engineering/consulting presence in Casablanca.','confidence':'HIGH','sourceIndexes':[i for i,x in enumerate(evidence) if 'rina' in norm(x.get('title','')+' '+x.get('snippet',''))][:5]})
    ...
    if refs:facts.append({'claim':'Public evidence contains the following procurement/contract reference(s): '+', '.join(refs)+'.','confidence':'MEDIUM','sourceIndexes':[i for i,x in enumerate(evidence) if any(r in (x.get('title','')+' '+x.get('snippet','')) for r in refs)][:5]})
    ...
    if 'rina' in n and any(x in n for x in ('office','subsidiary','casablanca','market entry')):actions.insert(0,'Track RINA Morocco as a strategic competitor market-entry signal and map its Morocco energy-transition, infrastructure and certification activity.')
    return {'project':project,'entities':signal.get('entities') or [],'facts':facts,'development':'Public-source enrichment completed; claims are limited to independently discoverable evidence.','interpretation':'The signal has been enriched according to its assigned research depth; unresolved claims remain explicitly flagged.','fichtnerImplication':'Potential relevance for technical advisory, owner’s engineering, grid, renewable-energy and energy-transition opportunities; exact scope requires verification.','recommendedActions':actions,'overallConfidence':'HIGH' if facts else 'MEDIUM','unresolved':[] if facts else ['Insufficient public evidence to verify the material claim.']}
